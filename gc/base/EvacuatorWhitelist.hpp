...
public:
	...
	add(MM_EvacuatorWhitespace *whitespace)
	{
		...
		if (MM_EvacuatorBase::max_scanspace_remainder < length) {
#if defined(EVACUATOR_DEBUG)
			Debug_MM_true(_tenure == _env->getExtensions()->isOld((omrobjectptr_t)whitespace));
			MM_EvacuatorWhitespace *address = whitespace;
#endif /* defined(EVACUATOR_DEBUG) */

			/* append whitespace or displace a leaf from the bottom of the pile */
			if (_count == MM_EvacuatorBase::max_whitelist) {

				/* whitelist is full -- look for a smaller leaf to displace with whitespace */
				MM_EvacuatorWhitespace * const endpoint[] { &_whitelist[MM_EvacuatorBase::max_whitelist - 1], &_whitelist[MM_EvacuatorBase::max_whitelist >> 1] };
				for (MM_EvacuatorWhitespace *leafspace = endpoint[0]; leafspace > endpoint[1]; leafspace -= 1) {
					if (leafspace->length() < length) {

						/* displace smaller leaf */
				...
				/* whitelist not full -- append whitespace without displacement */
				displaced = NULL;
				_count += 1;
			}

			/* if new whitespace was not displaced, sift it up the heap */
			if (displaced != whitespace) {

				_volume += length;
				const uintptr_t freeslot = _count - 1;
				_whitelist[freeslot] = whitespace;
				siftUp(freeslot);
			}
	...
	debug(MM_EvacuatorWhitespace *whitespace, const char* op)
	{
		if (_debugger->isDebugWhitelists()) {
			...
			char buf[512];
			uintptr_t sum = 0;
			uintptr_t len = omrstr_printf(buf, 512, "%5lu    %2llu:%7s[%c]; address:%llx; length:%llx; count:%llx; volume:%llx; top:%llx; discarded:%llx; flushed:%llx;",
					...
					(uint64_t)((NULL !=  _whitelist[0]) ? _whitelist[0]->length() : 0), (uint64_t)_discarded, (uint64_t)_flushed);
